...
https://www.youtube.com/watch?v=ji5_MqicxSo (Links to an external site.)

 (Randy Pausch Last Lecture: Achieving Your Childhood Dreams

and 
https://www.youtube.com/watch?v=oTugjssqOT0 (Links to an external site.)
...
My favorite new ones are actually http://pomodorotechnique.com/ignore-the-unicorn/ (Links to an external site.) and https://paidtoexist.com/counterintuitive-productivity/ (Links to an external site.) because I have ADD, and as http://pomodorotechnique.com/ignore-the-unicorn/ (Links to an external site.) describes, there's this “Fear of Missing Out” or fear of forgetting the idea/what needs to be done, because there are just so many things that need to get done.  Since I can think of each Pomodoro as a discreet unit, "... when your head is full of 50 things you could be doing right now, to know for certain that I’m in exactly the right place and doing exactly the right thing right now.”  Every time I have a distracting thought or idea, I just write it down before powering on with the rest of the Pomodoro.  The reason why I like "https://paidtoexist.com/counterintuitive-productivity/" (Links to an external site.) is because it encourages people to customize time management and productivity to suit their own personalities and figure out what works for each individual.  
...
pwd
= "print working directory", which tells the user which directory is currently being displayed/viewed.

ls = "list", which lists all of the contents of the current directory that you are in, which you found out through pwd.

mv = "move" which moves a file to a new location.  

Also, it will rename the file if you type "mv [old_name] [new_name]".

cd = "change directory", which moves the user to a different directory/lets the user view a different directory.

../ = it means "up one directory".  If you type "cd ../", your current folder will become the folder where the directory that you were just at "lives".  

Example, if you are in "/my_outer_directory/my_inner_directory", and you type "cd ../", your current directory will now be "/my_outer_directory".  

If you are in the root directory "/", typing "cd ../" will not do anything, because it is the outermost directory.  
...
= "remove directory", which deletes an empty directory.
